...
**Next human action:** open [docs/resume-android-freeze.md](docs/resume-android-freeze.md). A4 is complete (generic evidence collectors in `:core` and `apps/host` tested). Then say **execute A5** (Capability engine & OEM adapter boundary). Do not touch frozen www/API. Do not start Station or Knox. Do not run AGP Upgrade Assistant.
