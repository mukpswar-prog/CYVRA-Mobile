...
**Next human action:** open [docs/resume-android-freeze.md](docs/resume-android-freeze.md). A5 is complete (capability engine models & standard assessment engine in `:core` and host coordinator in `apps/host` tested). Then say **execute A6** (APK as `ANDROID_COMPONENT` boundary). Do not touch frozen www/API. Do not start Station or Knox. Do not run AGP Upgrade Assistant.
