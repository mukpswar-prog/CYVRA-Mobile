#!/usr/bin/env bash
#
# Codespaces / fresh machine path for G5 local ingest.
# Starts local Postgres (Neon stand-in), applies migrations, starts wrangler
# on :8787 if it is down, then runs scripts/test-local-evidence.sh.
...
# Never inherit a cloud API_URL. Local ingest is wrangler on :8787 only.
